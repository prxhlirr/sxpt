 ...
 *
 * 业务功能：
 * 1. 承载 SDK 进入考试模式时定位运行上下文所需的执行身份。
 * 2. 支持任务 ID、学生 ID 辅助校验，避免考试模式误用其他执行快照。
 *
 * 关键流程：
 * 1. Controller 从 query 参数组装请求对象。
 * 2. Service 基于该请求读取 runtime-context，并校验 sdkMode 必须为 EXAM。
 */
public class ExamContextQueryRequest {
