...
@TestPropertySource(properties = {
        "sxpt.execution.learning-context-controller.enabled=true",
...
class LearningContextControllerTests {
     ...
     */
    @Test
    void getLearningContextShouldReturnContext() throws Exception {
        LearningContextVO vo = buildLearningContextVO();
        when(learningContextService.getLearningContext(any(LearningContextQueryRequest.class))).thenReturn(vo);

        mockMvc.perform(get("/api/v1/sdk/learning/context")
                ...
                .andExpect(jsonPath("$.result.learningContextJson", is("{\"steps\":[{\"name\":\"学习步骤\"}]}")))
                .andExpect(jsonPath("$.result.deleted").doesNotExist());

        ArgumentCaptor<LearningContextQueryRequest> captor =
                ArgumentCaptor.forClass(LearningContextQueryRequest.class);
        verify(learningContextService).getLearningContext(captor.capture());
        LearningContextQueryRequest request = captor.getValue();
     ...
     */
    @Test
    void getLearningContextShouldRejectMissingExecutionId() throws Exception {
        mockMvc.perform(get("/api/v1/sdk/learning/context")
                        .header("Authorization", bearerToken())
                        .param("tenantId", "tenant_001"))
     ...
     * @return 学习模式上下文返回对象。
     */
    private LearningContextVO buildLearningContextVO() {
