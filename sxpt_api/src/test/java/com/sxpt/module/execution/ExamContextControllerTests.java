...
@TestPropertySource(properties = {
        "sxpt.execution.exam-context-controller.enabled=true",
...
class ExamContextControllerTests {
    ...
    @MockBean
    private ExamContextService examContextService;
     ...
     */
    @Test
    void getExamContextShouldReturnContext() throws Exception {
        ExamContextVO vo = buildExamContextVO();
        when(examContextService.getExamContext(any(ExamContextQueryRequest.class))).thenReturn(vo);

        mockMvc.perform(get("/api/v1/sdk/exam/context")
                ...
                .andExpect(jsonPath("$.result.sdkMode", is("EXAM")))
                .andExpect(jsonPath("$.result.examContextJson", is("{\"steps\":[{\"name\":\"考试步骤\"}]}")))
                .andExpect(jsonPath("$.result.hintDisabled", is(true)))
                .andExpect(jsonPath("$.result.deleted").doesNotExist());

        ArgumentCaptor<ExamContextQueryRequest> captor =
                ArgumentCaptor.forClass(ExamContextQueryRequest.class);
        verify(examContextService).getExamContext(captor.capture());
        ExamContextQueryRequest request = captor.getValue();
     ...
     */
    @Test
    void getExamContextShouldRejectMissingExecutionId() throws Exception {
        mockMvc.perform(get("/api/v1/sdk/exam/context")
                        .header("Authorization", bearerToken())
                        .param("tenantId", "tenant_001"))
     ...
     * @return 考试模式上下文返回对象。
     */
    private ExamContextVO buildExamContextVO() {
        ...
        vo.setSdkMode("EXAM");
        vo.setExamContextJson("{\"steps\":[{\"name\":\"考试步骤\"}]}");
        vo.setResourceSnapshotJson("[{\"id\":\"resource_001\"}]");
        vo.setEvaluationSnapshotJson("[{\"id\":\"rule_001\"}]");
        vo.setSilentCapturePolicyJson("{\"silentCapture\":true}");
